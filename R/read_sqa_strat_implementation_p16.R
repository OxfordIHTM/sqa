#'
#' Read SQA strategic plan implementation page 16
#' 

read_sqa_strategy_implementation_p16 <- function(df_text) {
  ptext <- df_text[[16]][5:21] |>
    (\(x) x[x != ""])() |>
    stringr::str_replace_all(pattern = "^\\s{48,}", replacement = ";;") |>
    stringr::str_replace_all(pattern = "^\\s{26,}", replacement = ";") |>
    ...
    (\(x)
      {
        x[c(1:2, 12:16)] <- paste0(x[c(1:2, 12:16)], ";;;;;;;;")
        x
      }
    )() |>
    (\(x)
      {
        x[c(5:6, 8, 11)] <- paste0(x[c(5:6, 8, 11)], ";;;;;;;")
        x
      }
    )() |>
    (\(x)
      {
        x[c(4, 7)] <- paste0(x[c(4, 7)], ";;")
        x
      }
    )() |>
  ...
  performance_indicator <- df$X3 |>
    paste(collapse = " ") |>
    trimws() |>
    stringr::str_replace_all(pattern = " Number", replacement = ";Number") |>
    stringr::str_replace_all(
      pattern = " Compliance", replacement = ";Compliance"
    ) |>
    stringr::str_split(pattern = ";") |>
    unlist() |>
    (\(x) c("", x))()
